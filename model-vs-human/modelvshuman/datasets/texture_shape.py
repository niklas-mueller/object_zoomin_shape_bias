...
__all__ = ["original", "greyscale", "texture", "edge", "silhouette",
           "cue_conflict", "cue_conflict_crop", "cue_conflict_full"]
...
def _get_crop_dataset(name, *args, **kwargs):
    params = TextureShapeParams(path=pjoin(c.CROP_DATASET_DIR, name), image_size=kwargs['image_size'])
...
def _get_dataset(name, *args, **kwargs):
    params = TextureShapeParams(path=pjoin(c.DATASET_DIR, name), image_size=kwargs['image_size'])
    return Dataset(name=name,
                   params=params,
                   loader=PytorchLoader,
                   *args,
...
@register_dataset(name="cue-conflict-crop")
def cue_conflict_crop(*args, **kwargs):
    return _get_crop_dataset("crop", *args, **kwargs)

@register_dataset(name="cue-conflict-full")
def cue_conflict_full(*args, **kwargs):
    return _get_crop_dataset("full", *args, **kwargs)
