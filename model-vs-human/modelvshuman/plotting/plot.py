...
def plot_shape_bias_matrixplot(datasets,
    ...
    plt.tick_params(axis='y',
                    ...
                    right=False,
                    labelleft=False)
    ax.set_ylabel("Shape categories", labelpad=30, fontsize=fontsize, fontweight='bold')
    ax.set_xlabel("Texture bias", fontsize=fontsize, labelpad=4, fontweight='bold')
    ax_top.set_xlabel("Shape bias", fontsize=fontsize, labelpad=8, fontweight='bold')
    # ax.set_xlabel("Fraction of 'texture' decisions", fontsize=fontsize, labelpad=14)
    # ax_top.set_xlabel("Fraction of 'shape' decisions", fontsize=fontsize, labelpad=14)
    ...
    # arrows on x axes
    offset = 1.1
    plt.arrow(x=0, y=-(1+offset), dx=1, dy=0, fc='black',
              head_width=0.4, head_length=0.03, clip_on=False,
              length_includes_head=True, overhang=0.5)
    plt.arrow(x=1, y=num_classes + offset, dx=-1, dy=0, fc='black',
              head_width=0.4, head_length=0.03, clip_on=False,
              length_includes_head=True, overhang=0.5)
    ...
    for dmaker in decision_maker_fun(df):
        ...
        print(f"shape bias: {result_df['shape-bias']} for {df_selection['subj'].unique()}")
    ...
    figure_path = pjoin(result_dir, f"{ds.name}_shape-bias_matrixplot.png")
...
def plot_shape_bias_boxplot(datasets,
    ...
    figure_path = pjoin(result_dir, f"{ds.name}_shape-bias_boxplot.png")
    fig.savefig(figure_path)
    plt.close()
