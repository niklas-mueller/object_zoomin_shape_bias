...
from .models import pytorch_model_zoo, list_models
...
def load_dataset(name, *args, **kwargs):
    default_kwargs = {"batch_size": 16, "num_workers": 4, 'grayscale': False, 'resize': False}
    kwargs = {**default_kwargs, **kwargs}
    logger.info(f"Loading dataset {name}")
    supported_datasets = dataset_module.list_datasets()
    supported_datasets['crop'] = 'cue_conflict_crop'
    supported_datasets['full'] = 'cue_conflict_full'
    ...
    elif os.path.exists(join(c.CROP_DATASET_DIR, name)):
        return eval(f"dataset_module.{module_name}")(*args, **kwargs)
